...
//////////////////////////////////////////////////////////////////////
  ...
  #define MAESTRO_ENABLED                            // Comment this out if not using the Maestro Servo Controller
    ...
    #define MAESTRO_CONTROLLER_KYBER                // Uncomment this out if you are using the Kyber/Microcontroller using the Maestro Library
    // #define MAESTRO_CONTROLLER_KYBER_LOCAL          // Uncomment this if the Kyber is plugged into the local WCB
    ...
    uint8_t localMaestroID = 0x02;                  // The Maestro ID that is connected to the local WCB
  #endif
  ...
  String DEFAULT_ESPNOWPASSWORD = "ESPNOW_KyberTest";
  ...
  #ifdef MAESTRO_CONTROLLER_KYBER_LOCAL           // Checks to see if you are recieving commands from Kyber directly  
    #define SERIAL1_DEFAULT_BAUD_RATE 57692         // Set the baud rate to receive commands from the Kyber/Pololu Library (Using 57692 per Kyber recommendations)
  #else
    #define SERIAL1_DEFAULT_BAUD_RATE 9600          // Default Baud Rate for Serial 1 if not using Kyber/Polulu Library
  #endif
  ...
  #define CHAIN_DEFAULT_BAUD_RATE 57692            // Used for Maestro (Not sure if needed or not yet)

  
    // Default Serial Broadcast State   ******THESE ARE ONLY CORRECT UNTIL YOU CHANGE THEM VIA THE COMMAND LINE.  ONCE CHANGED, THEY MAY NOT MATCH THIS NUMBER.
  // The correct broadcast state will be shown on the serial console on bootup.
  #ifdef MAESTRO_CONTROLLER_KYBER_LOCAL           // Checks to see if you are recieving commands from the Pololu Library 
  ...
  const uint8_t umac_oct2 = 0xF1;     // unique MAC address for the 2nd Octet
