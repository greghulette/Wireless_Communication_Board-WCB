...
void PersistentStringHandler::listKeys() {
    nvs_opaque_iterator_t *it = nullptr; // Declare the iterator correctly
    esp_err_t err = nvs_entry_find(NVS_DEFAULT_PART_NAME, namespaceName, NVS_TYPE_STR, &it);
    ...
    while (err == ESP_OK) {
        ...
        err = nvs_entry_next(&it); // Pass iterator correctly
    }
