...
// Stop style for ;H,STOP / ;H,STOPEMOTE: "NOW" (also the empty field, i.e. the
// pre-existing two-field form) or "GRACEFUL". "" = not a style, so the caller
// can reject a typo instead of cutting the vocalizer off by accident.
static String hcrStopStyle(const String &t) {
  String u = t; u.trim(); u.toUpperCase();
  if (u == "" || u == "NOW" || u == "N" || u == "HARD")       return "NOW";
  if (u == "GRACEFUL" || u == "G" || u == "SOFT")             return "GRACEFUL";
  return "";
}
...
void processHCRRuntimeCommand(const String &message) {
  ...
  // STOP / STOPEMOTE take an optional style: NOW (default, unchanged) cuts the
  // vocalizer off with <PSV>; GRACEFUL lets it finish the sound it is making with
  // <PSG>. Before this, nothing in the ;H layer could reach <PSG>, and declaring
  // the port with ?HCR,PORT is what made it unreachable by raw frame too (the
  // port leaves the broadcast loop). Issue #15. An unrecognised style is rejected
  // rather than treated as NOW: silently cutting a droid off is the wrong default
  // for a typo, and the old callers pass no style at all.
  if (vU == "STOP" || vU == "STOPEMOTE") {
    const String st = hcrStopStyle(hcrField(body, 1));
    if (st == "") {
      Serial.printf("[HCR] Usage: ;H,%s[,NOW|GRACEFUL]\n", vU.c_str());
      return;
    }
