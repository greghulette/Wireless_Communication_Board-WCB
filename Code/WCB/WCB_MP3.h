...
//  Configure with : ?MP3,S<port>:<baud>         (e.g. ?MP3,S2:9600)
//  Operate  with  : ;A,PLAY,<n>  ;A,STOP  ;A,VOL,<n>  etc.
//  Callbacks      : ?MP3,ONFIN,<key>  — run stored-cmd on track finish
//                   ?MP3,ONERR,<key>  — run stored-cmd on error
// -----------------------------------------------------------------------
...
struct MP3Config {
  ...
  char     onFinCmd[24];  // stored-command key to run when track finishes ('X')
  char     onErrCmd[24];  // stored-command key to run when error ('E')
};

extern MP3Config mp3Config;
extern uint8_t   mp3Volume;  // shadow of last-sent volume byte (0=max loud, 255=silent)
