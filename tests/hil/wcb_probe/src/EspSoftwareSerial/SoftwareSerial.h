...
// WCB vendored + patched copy of EspSoftwareSerial 8.1.0 - see README.md here for the patch list.
// WCB patch: the include guard is renamed, and __SoftwareSerial_h is deliberately left UNdefined, so a stock
// SoftwareSerial.h (angle-bracket include) pulled into the same build (a stray include, a library that includes it) fails loudly - either
// the #error below or a class redefinition - instead of silently linking two different receivers.
#ifndef WCB_VENDORED_SOFTWARESERIAL_H
#define WCB_VENDORED_SOFTWARESERIAL_H
#ifdef __SoftwareSerial_h
#error "the stock EspSoftwareSerial SoftwareSerial.h is included - the WCB builds only its vendored src/EspSoftwareSerial"
#endif
#include <sdkconfig.h>
// WCB patch (tracker #78): ESP32 erratum GPIO-3.14 (every classic-ESP32 revision, no fix scheduled) - on GPIO0-31 an
// EDGE interrupt is lost when the GPIO ISR's STATUS read / W1TC clear for another pin lands on it. Measured on a WCB
// (HIL softrx.erratum_pairs, 2026-09-23): 227 of 10125 lines lost when two or three S3-S5 pins took edges ~1.5-3 us
// apart, every single-port line exact. Level interrupts are not lost that way (the status re-asserts while the
// condition holds), so on the classic ESP32 rxBitISR runs on a LEVEL interrupt whose polarity it flips after every
// edge (Espressif's documented workaround). The ESP32-S3 has no such erratum and keeps the stock CHANGE interrupt.
#if CONFIG_IDF_TARGET_ESP32
#define ESPSWSERIAL_LEVEL_RX 1
#else
#define ESPSWSERIAL_LEVEL_RX 0
#endif
...
public:
    ...
    /// WCB patch (tracker #78): true while RX is running on the level-emulated interrupt (classic ESP32, rxBitISR).
    /// False when RX is off, on the ESP32-S3 (CHANGE) and above ~74880 baud (rxBitSyncISR, FALLING edge).
    bool rxLevelTriggered() const {
        return ESPSWSERIAL_LEVEL_RX && m_rxEnabled && m_bitTicks >= microsToTicks(1000000UL / 74880UL);
    }
...
private:
    ...
    volatile bool m_rxIsrLevel = true;   // WCB patch (#78): raw pad level the level-emulated rxBitISR last recorded
#endif
...
#endif // WCB_VENDORED_SOFTWARESERIAL_H
